  ...
  const steps = [
    {
      ...
      description: "Simply point your camera or upload photos of your items. No professional photography required.",
    ...
    <div className="bg-background py-16 px-6 md:px-12 lg:px-24">
      ...
      <section className="mt-32 rounded-[3rem] bg-blue-50/50 border border-blue-100/50 py-20 px-6 text-center max-w-4xl mx-auto">
          <div className="flex justify-center -space-x-3 mb-8">
            {[1, 2, 3].map((i) => (
              <div key={i} className="relative w-12 h-12 rounded-full border-4 border-white overflow-hidden bg-slate-200">
                 <Image 
                  src={`https://i.pinimg.com/736x/92/24/eb/9224eb869902b64ca066f5b62b682e2f.jpg`} 
                  alt="user avatar" 
                  fill 
                  className="object-cover" 
                 />
            ...
            <div className="w-12 h-12 rounded-full border-4 border-white bg-primary flex items-center justify-center text-[10px] font-bold text-primary-foreground">
              +10k
            </div>
          </div>
        <h3 className="text-3xl font-bold font-montserrat mb-4 text-foreground">Ready to Digitise?</h3>
        <p className="text-foreground/50 mb-10 max-w-sm mx-auto">Join thousands who have revolutionized their morning routine.</p>
        
        <div className="flex flex-col gap-4 max-w-xs mx-auto">
          <Link href="/waitlist">
            <button className="bg-primary text-primary-foreground font-bold py-5 w-full rounded-2xl flex items-center justify-center gap-2 shadow-lg shadow-primary/20 hover:scale-[1.02] transition-transform">
              START YOUR CLOSET <ArrowRight size={18} />
            </button>
          </Link>
          <Link href="/waitlist">
            <button className="bg-white border border-foreground/10 text-foreground font-bold py-5 w-full rounded-2xl hover:bg-foreground/5 transition-colors">
              TALK TO A STYLIST
            </button>
          </Link>
        </div>
